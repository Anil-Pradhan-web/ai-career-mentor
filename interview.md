...
- `backend/app/core/database.py`: SQLAlchemy engine/session setup and `get_db` dependency.
- `backend/app/core/rate_limit.py`: per-user daily limits for AI features using Redis if `REDIS_URL` exists, otherwise in-memory fallback.
...
Answer: Interview 5/day, resume 6/day, roadmap 5/day, full analysis 5/day, and LinkedIn 5/day.
...
61. How do you prevent LLM cost abuse?
Answer: The platform uses authenticated routes, per-user daily AI feature limits, request-level SlowAPI limits, and frontend feedback for 429 responses.
