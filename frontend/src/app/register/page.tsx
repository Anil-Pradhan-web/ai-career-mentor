...
export default function RegisterPage() {
  ...
  const strengthColors = ["var(--border-default)", "#ef4444", "#f59e0b", "#10b981"];
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden p-6" style={{ background: "var(--bg-base)" }}>
      <div className="absolute pointer-events-none" style={{ top: "-30%", right: "-10%", width: "60vw", height: "60vw", background: "radial-gradient(circle, rgba(139, 92, 246, 0.03) 0%, transparent 70%)", filter: "blur(80px)" }} />
      <div className="absolute pointer-events-none" style={{ bottom: "-30%", left: "-10%", width: "60vw", height: "60vw", background: "radial-gradient(circle, rgba(59, 130, 246, 0.04) 0%, transparent 70%)", filter: "blur(80px)" }} />
      ...
      <div className="flex w-full max-w-4xl overflow-hidden z-10 animate-fade-up" style={{ background: "var(--bg-card)", border: "1px solid var(--border-default)", borderRadius: "var(--radius-2xl)", boxShadow: "0 24px 48px rgba(0, 0, 0, 0.5)" }}>
        ...
        <div className="flex-1 flex flex-col justify-center auth-right-panel" style={{ padding: "36px" }}>
          <div className="max-w-sm mx-auto w-full">
            <div className="mb-7">
              <h1 className="font-display font-bold mb-1" style={{ fontSize: "1.5rem", color: "var(--fg-primary)", letterSpacing: "-0.03em" }}>Create Account</h1>
              <p style={{ color: "var(--fg-secondary)", fontSize: "0.8125rem" }}>
                Have an account? <Link href="/login" style={{ color: "#a78bfa", fontWeight: 700, textDecoration: "underline", textUnderlineOffset: "3px" }}>Sign In</Link>
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit}>
              <AuthInput label="Full Name" icon={User} type="text" placeholder="John Doe" value={name} onChange={(e) => setName(e.target.value)} required />
              ...
              <div className="relative">
                <AuthInput label="Password" icon={Lock} type={showPw ? "text" : "password"} placeholder="Min. 6 characters" value={password} onChange={(e) => setPassword(e.target.value)} required />
                ...
                {password.length > 0 && (
                  <div className="flex gap-1 mt-1.5">
                    {[1, 2, 3].map(i => (
                      <div key={i} style={{ flex: 1, height: "2px", borderRadius: "99px", background: i <= pwStrength ? strengthColors[pwStrength] : "var(--border-default)", transition: "background 0.3s" }} />
                    ))}
                  </div>
                )}
              </div>
              <div className="mt-5">
                <AuthButton loading={loading}>Get Started <ArrowRight size={15} /></AuthButton>
              </div>
            </form>
        ...
        <div className="hidden lg:flex flex-1 flex-col justify-between p-8 auth-left-panel" style={{ background: "linear-gradient(180deg, rgba(139, 92, 246, 0.03) 0%, transparent 100%)", borderLeft: "1px solid var(--border-subtle)" }}>
          ...
          <div className="mt-auto">
            <h2 className="font-display font-bold leading-tight mb-3" style={{ fontSize: "1.5rem", color: "var(--fg-primary)", letterSpacing: "-0.03em" }}>
              Start Free.<br /><span className="gradient-text">Grow Fast.</span>
            </h2>
            ...
            <div className="flex flex-col gap-2">
              {PERKS.map((perk, i) => (
                <div key={i} className="flex items-center gap-2.5" style={{ padding: "10px 12px", background: "var(--bg-surface)", border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-md)" }}>
                  <CheckCircle2 size={14} style={{ color: "var(--accent-emerald)", flexShrink: 0 }} />
                  <span style={{ fontSize: "0.75rem", color: "var(--fg-primary)", fontWeight: 500 }}>{perk}</span>
                </div>
              ))}
            </div>
          </div>
